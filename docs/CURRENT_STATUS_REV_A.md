...
kicad-cli pcb drc --severity-all
  ...
  27 expected unconnected/ratsnest items because the board is not routed
```
...
`BAT_RAW` is now routed from the battery XT30 pad to the LM66100/XIAO battery-isolation input through a wider inner-layer branch with a local U2 entry, and that branch is tied to the local battery capacitor island. The final U1 BAT escape and BATP/R202 sense branch remain open.
